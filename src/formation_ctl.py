...
class Reynolds:
    def __init__(self):
        ...
        self.visualize_formation = rospy.get_param("~final_formation/visualize")
        ...
        self.switching_topology = rospy.get_param("~switching_topology")
        ...
        self.visualize_traj = rospy.get_param('~visualize_traj')
...
# -------------------------------------OTHER PARAMS------------------------------------
        ...
        self.robot_thetas = {i: 0 for i in range(self.n_boids)}
    ...
    def odom_callback(self, data, robot_id):
        ...
        self.robot_thetas[robot_id] = pose_msg_to_state(data)[2]
    ...
    def run(self, event):
        if self.graph.is_graph_init():        
            ...
            # NOTE: I had to use a hard arrival condition to stop all agents since the steer to avoid behavior was not working properly when added.
        ...
        if self.graph.is_graph_init() and self.visualize_traj:
    ...
    def publish_boid_trajectory(self):
        positions = self.graph.get_positions()
        if len(positions) != len(self.robot_thetas):
            return
        for i in range(self.graph.G.number_of_nodes()):  
            pose = PoseStamped()
            ...
            # use i as the boid id and get the theta 
            quaternion = tf.transformations.quaternion_from_euler(0.0, 0.0, self.robot_thetas[i])
    ...
    def _visualize(self, event):
        ...
        if self.formation_positions_ is not None and self.visualize_formation:
            positions_ = self.create_desired_formation() if self.k > 0 else self.create_desired_formation(stubborn_agents=False)
        ...
        marker = Marker(
                ...
                scale=Vector3(self.goal_radius*2.5, self.goal_radius*2.5, 0.3),
                lifetime=rospy.Duration(0),
            )
